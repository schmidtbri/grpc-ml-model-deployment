...
clean-build:  ## clean build artifacts
	rm -rf build
	rm -rf dist
	rm -rf model_grpc_service.egg-info
...
test-reports: clean-pyc clean-test ## Run unit test suite with reporting
	...
	mkdir ./reports/badge
	python -m coverage run --source model_grpc_service -m pytest --verbose --color=yes --html=./reports/unit_tests/report.html --junitxml=./reports/unit_tests/report.xml $(TEST_PATH)
